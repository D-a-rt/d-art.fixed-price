...
type create_editions_param = assign_edition_param list

let create_txs_editions (param, storage : create_editions_param * nft_token_storage) : minted =
    let seed1 : minted = {
        storage = storage;
        reversed_txs = ([] : transfer_destination_descriptor list);
    } in
    ...
    List.fold
        (fun (acc, t : minted * assign_edition_param) ->
...
let mint_edition_set (param, storage : create_editions_param * nft_token_storage) : operation list * nft_token_storage =
(* update ledger *)
...
[@inline]
let token_id_to_edition_id (token_id, storage : token_id * editions_storage) : edition_id = (token_id/storage.max_editions_per_run)
