...
let fail_if_not_gallery (storage : admin_storage) : unit =
...
(* Fails if sender is not gallery *)
let fail_if_already_originated (storage : admin_storage) : unit =
    if Big_map.mem Tezos.sender storage.galleries
    then failwith "ALREADY_ORIGINATED"
    else unit

(* Fails if serie origination paused *)
let fail_if_origination_paused (storage : admin_storage) : unit =
