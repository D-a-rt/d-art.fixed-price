...
let test_buy_fixed_price_token_success =
    ...
    match result with
        Success _gas -> (
            ...
            in
            let new_buyer_balance = Test.get_balance buyer in
            
            let () = let match (buyer_bal - new_buyer_balance) with
                None -> "yup"
                | Some teee ->  Test.log (teee - 213210368547757mutez, "Buyer balance") 
            in
