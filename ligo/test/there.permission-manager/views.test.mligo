...
#include "../../there.permission-manager/views.mligo"
...
// -- Is auction house manager --

let test_is_auction_house_manager =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None : address option), true) in
    let contract = Test.to_contract contract_add  in

    let auction_house = Test.nth_bootstrap_account 1 in
    let not_auction_house_manager = Test.nth_bootstrap_account 2 in
    
    let admin = Test.nth_bootstrap_account 0 in
    let () = Test.set_source admin in

    let _gas = Test.transfer_to_contract_exn contract (Admin (Add_auction_house_manager (auction_house))) 0tez in

    let strg = Test.get_storage contract_add in

    let is_auction_house_manager_true = is_auction_house_manager (auction_house, strg) in
    let is_not_auction_house_false = is_auction_house_manager (not_auction_house_manager, strg) in

    let () = assert_with_error (is_auction_house_manager_true = true) "Views - Is auction_house : This test should pass, correct auction_house specified" in
    let () = assert_with_error (is_not_auction_house_false = false) "Views - Is auction_house : This test should pass, uncorrect auction_house specified" in
    "Passed"
