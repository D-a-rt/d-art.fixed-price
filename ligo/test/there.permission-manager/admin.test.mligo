...
// -- Add_auction_house_manager --

// No amount
let test_add_auction_house_no_amount =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None: address option), true) in
    let contract = Test.to_contract contract_add  in

    let new_auction_house_manager = Test.nth_bootstrap_account 1 in

    let result = Test.transfer_to_contract contract (Admin (Add_auction_house_manager (new_auction_house_manager))) 1tez in

    match result with
            Success _gas -> failwith "Admin -> Add_auction_house_manager - No amount : This test should fail"
        |   Fail (Rejected (err, _)) -> (
            let () = assert_with_error ( Test.michelson_equal err (Test.eval "AMOUNT_SHOULD_BE_0TEZ") ) "Admin -> Add_auction_house_manager - No amount : Should not work if amount specified" in
                "Passed"
            )
        |   Fail _ -> failwith "Internal test failure"    

// Not an admin
let test_add_auction_house_not_admin =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None: address option), true) in
    let contract = Test.to_contract contract_add  in

    let new_auction_house_manager = Test.nth_bootstrap_account 1 in
    let () = Test.set_source new_auction_house_manager in

    let result = Test.transfer_to_contract contract (Admin (Add_auction_house_manager (new_auction_house_manager))) 0tez in

    match result with
            Success _gas -> failwith "Admin -> Add_auction_house_manager - Not admin : This test should fail"
        |   Fail (Rejected (err, _)) -> (
            let () = assert_with_error ( Test.michelson_equal err (Test.eval "NOT_AN_ADMIN") ) "Admin -> Add_auction_house_manager - Not admin : Should not work if not admin" in
                "Passed"
            )
        |   Fail _ -> failwith "Internal test failure"    

// Success
let test_add_auction_house_success =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None: address option), true) in
    let contract = Test.to_contract contract_add  in
    ...
    match result with
            Success _gas -> (
                let new_str = Test.get_storage contract_add in
                match Big_map.find_opt new_auction_house_manager new_str.auction_house_managers with
                        Some _ -> "Passed"
                    |   None -> "Admin -> Add_auction_house_manager - Success : This test should pass (no space saved)"
            )
        |   Fail (Rejected (_err, _)) -> failwith "Admin -> Add_auction_house_manager - Success : This test should pass"
        |   Fail _ -> failwith "Internal test failure"



// Already minter
let test_add_auction_house_already_minter =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None : address option), true) in
    let contract = Test.to_contract contract_add  in
    ...
    let _gas = Test.transfer_to_contract_exn contract (Admin (Add_auction_house_manager (new_auction_house_manager))) 0tez in
    let result = Test.transfer_to_contract contract (Admin (Add_auction_house_manager (new_auction_house_manager))) 0tez in

    match result with
            Success _gas -> failwith "Admin -> Add_auction_house_manager - Already space : This test should fail"
        |   Fail (Rejected (err, _)) -> (
            let () = assert_with_error ( Test.michelson_equal err (Test.eval "ALREADY_AUCTION_HOUSE_MANAGER") ) "Admin -> Add_auction_house_manager - Already space : Should not work if aldready a space" in
                "Passed"
            )
        |   Fail _ -> failwith "Internal test failure"


// -- Remove_auction_house_manager --

// No amount
let test_remove_auction_house_no_amount =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None: address option), true) in
    let contract = Test.to_contract contract_add  in

    let old_auction_house_manager = Test.nth_bootstrap_account 1 in

    let result = Test.transfer_to_contract contract (Admin (Remove_auction_house_manager (old_auction_house_manager))) 1tez in

    match result with
            Success _gas -> failwith "Admin -> Remove_auction_house_manager - No amount : This test should fail"
        |   Fail (Rejected (err, _)) -> (
            let () = assert_with_error ( Test.michelson_equal err (Test.eval "AMOUNT_SHOULD_BE_0TEZ") ) "Admin -> Remove_auction_house_manager - No amount : Should not work if amount specified" in
                "Passed"
            )
        |   Fail _ -> failwith "Internal test failure"    

// Not an admin
let test_remove_auction_house_not_admin =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None: address option), true) in
    let contract = Test.to_contract contract_add  in
    ...
    let result = Test.transfer_to_contract contract (Admin (Remove_auction_house_manager (new_auction_house_manager))) 0tez in

    match result with
            Success _gas -> failwith "Admin -> Remove_auction_house_manager - Not admin : This test should fail"
        |   Fail (Rejected (err, _)) -> (
            let () = assert_with_error ( Test.michelson_equal err (Test.eval "NOT_AN_ADMIN") ) "Admin -> Remove_auction_house_manager - Not admin : Should not work if not admin" in
                "Passed"
            )
        |   Fail _ -> failwith "Internal test failure"    

// Not a space
let test_remove_auction_house_not_found =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None: address option), true) in
    let contract = Test.to_contract contract_add  in
    ...
    match result with
            Success _gas -> "Passed" 
        |   Fail (Rejected (_err, _)) -> failwith "Admin -> Remove_auction_house_manager - Minter not found : This test should pass"
        |   Fail _ -> failwith "Internal test failure"

// Success
let test_remove_auction_house_success =
    let contract_add, _ = PM_STR. get_permission_manager_contract((None : address option), true) in
    let contract = Test.to_contract contract_add  in
    ...
    match result with
            Success _gas -> (
                let new_str = Test.get_storage contract_add in
                match Big_map.find_opt new_auction_house_manager new_str.auction_house_managers with
                        Some _ -> "Admin -> Remove_auction_house_manager - Success : This test should pass (error: minter not removed)"
                    |   None -> "Passed"
            )
        |   Fail (Rejected (_err, _)) -> failwith "Admin -> Remove_auction_house_manager - Success : This test should pass"
        |   Fail _ -> failwith "Internal test failure"
