...
### Complementary informations regarding synthtic assets in the context of this contract:
...
Before the exhibition period starts, the synthetic assets should be listed on a secondary market. If a synthetic asset is sold, the real FA2 tokens associated with that asset are transferred to the buyer, and the curator is credited with a commission for the sale. The commission is split according to the percentages specified in the commission_splits field of the admin_storage.
...
type creator_entrypoints =
    | Accept_invitation of fa2_base list
    | Accept_listing of nat list
    | Claim_artwork of nat list
...
type admin_entrypoints =
    | Send_invitation of (fa2_base * minter) list
    | Remove_invitation of (fa2_base * minter) list
    ...
    | Edit_listing of sale_configuration
    | Start_grace_period
    | Reset_exhibition
...
The `Start_grace_period` is the period during wich the creator can pull-out from the exhibition one last time.
