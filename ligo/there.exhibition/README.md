...
The purpose of the exhibition contract is to enable curators to create a time based exhibition by inviting creators to collaborate with one or more of there artworks (FA2 token using the there.fa2-editions contracts or any FA2 not using the multi-asset contract).
...
### Steps to start an exhibition

- Originate a contract using the there.art-factories/exhibition_factory.mligo contract.
- Set a start-date
- Set an end-date
- Set a commission
- Set a grace period (defined [here](#grace-period))

- Invite creators to collaborates with one or more artworks
- List the synthetic assets (auto-matically generated)
- Start the grace period
- Wait for the start date (In case any issues happened you can always reset the exhibition)
...
### Complementary informations about the grace period: {#grace-period}

The grace period is defined by the curator, it is a minimum of 24h. During that period the curator will not be able to change any of the parameters of the exhibition and the creators will have the ability to pull-out in case they have a second thought about the exhibition. Before the start date the curator can always reset the contract.

### Complementary informations on reseting the exhibition contract:

Reseting the contract will remove all the preset listings, return the tokens to the creators, pending_invitations and set the `grace_started` arguments to nil.
