...
type fixed_price_entrypoints =
    ...
    | RevokeSale of revoke_sales_param
    | CreateDrop of drop_configuration
    | RevokeDrop of revoke_drops_param
...
let create_sales (sale_configuration, storage : sale_configuration * storage) : return =
    ...
    let () = assert_msg (Tezos.sender = sale_configuration.seller, "Only seller can create a sale") in
    ...
    let create_sale : (storage * sale_info ) -> storage =
        fun (strg, sale_param : storage * sale_info ) ->
            let () = assert_msg (sale_param.price > 10000mutez, "Price should be greater than 0.1tez" ) in
            ...
            let () = assert_msg (not Big_map.mem (sale_param.fa2_token, Tezos.sender) storage.for_sale, "Token already for sale, only update or revoke are authorized") in

            {
                storage with
                for_sale = Big_map.add ({ address = sale_param.fa2_token.address; id = sale_param.fa2_token.id; }, sale_configuration.seller) fixed_price_sale_values storage.for_sale;
                admin.signed_message_used = Big_map.add sale_configuration.authorization_signature unit storage.admin.signed_message_used
            }
    in
...
let buy_fixed_price_token (buy_token, storage : buy_token * storage) : return =
    ...
    operation_list, { storage with for_sale = Big_map.remove (buy_token.fa2_token, buy_token.seller) storage.for_sale; admin.signed_message_used = Big_map.add buy_token.authorization_signature unit storage.admin.signed_message_used }
...
let create_drops (drop_configuration, storage : drop_configuration * storage) : return =
    ...
    let create_drop : ( storage * drop_info ) -> storage =
        fun (strg, drop_param : storage * drop_info ) ->
            ...
            let () = assert_msg (drop_param.price > 10000mutez, "Price should be greater than 0.1 tez" ) in
...

            {
                ...
                admin.signed_message_used = Big_map.add drop_configuration.authorization_signature unit storage.admin.signed_message_used
            } 
    in
...
let buy_dropped_token (buy_token, storage : buy_token * storage) : return =
    ...
    let new_strg = { storage with drops = Big_map.remove (buy_token.fa2_token, buy_token.seller) storage.drops; admin.signed_message_used = Big_map.add buy_token.authorization_signature unit storage.admin.signed_message_used } in
...
let fixed_price_tez_main (p , storage : fixed_price_entrypoints * storage) : return = match p with
    ...
    | UpdateSale sale_configuration -> update_sales (sale_configuration, storage)
    | RevokeSale token_info -> revoke_sales (token_info, storage)

    // Drops entrypoints
    | CreateDrop drop_configuration -> create_drops (drop_configuration, storage)
    | RevokeDrop drop_info -> revoke_drops (drop_info, storage)
