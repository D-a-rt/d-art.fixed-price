...
let permission_manager_main (param, storage : art_permission_manager * storage)  : (operation list) * storage = 
    ...
    match param with
        ...
        |   Accept_admin_invitation param ->
                ...
                then ([] : operation list), { storage with admin_str.pending_admin = (None : address option); admin_str.admin = Tezos.get_sender() }
                else ([] : operation list), { storage with admin_str.pending_admin = (None : address option) }
